...
const AccordionPostContactMessage = (props) => {
  const [cookies, setCookie] = useCookies();
  const myId = cookies.user._id;
  const { message } = props;
  const { poster, content, createdAt, category } = message;
  ...
  return (
    ...
    >
      <div className=" flex flex-row w-full justify-between items-center p-3 font-normal cursor-pointer">
        <div className=" text-base ml-2">{poster.name.lastNameGanji}さん</div>
        {!active && (
          <div className=" overflow-hidden p-3 whitespace-nowrap w-[70%] max-h-full text-lg text-ellipsis">
            {content}
          </div>
        )}
      ...
      {active && (
        <span className="p-3 w-[800px] font-normal whitespace-pre-wrap break-words mx-auto">
          {content}
        </span>
      )}
    </div>
...
export default AccordionPostContactMessage;
