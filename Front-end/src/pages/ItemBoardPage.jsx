...
const ItemBoardPage = () => {
  ...
  const handleResize = () => {
    ...
    // if (currentWidth > 0 && currentWidth < 640) {
    //   setBreakpoint("xs");
    //   console.log("I am here XS");
    // }
    // if (currentWidth >= 640 && currentWidth < 768) {
    //   setBreakpoint("sm");
    //   console.log("I am here SM");
    // }
    // if (currentWidth >= 768 && currentWidth < 976) {
    //   setBreakpoint("md");
    //   console.log("I am here MD");
    // }
    // if (currentWidth >= 976 && currentWidth < 1440) {
    //   setBreakpoint("lg");
    //   console.log("I am here LG");
    // }
    // if (currentWidth >= 1440) {
    //   setBreakpoint("xl");
    //   console.log("I am here XL");
    // }
    setTimeout(() => {
      console.log(isXL);
    }, 2000);
  ...
  return (
    <div className="flex flex-col items-center clear-both bg-[#F1F1F1] pb-20">
      <div
        className={`flex justify-center relative w-full bg-[#F1F1F1] pb-8 ${
      ...
      >
          ...
          {Object.keys(myImage).map((key, i) => (
            ...
            >
              <Category
                ...
                img={myImage[key]}
                onClick={(e) =>
                  goToItemListPageWithFilterContent(e, "filterByCategory")
                }
                alt={i}
              />
            </div>
        ...
        <div className="flex flex-col items-center">
          <div className="pt-[60px] font-medium">
            <div className="flex gap-[22px]">
              <span
                className="text-[#02540A] w-[48px] pb-[10px] cursor-pointer"
                onClick={(e) =>
                  goToItemListPageWithFilterContent(e, "filterByProvince")
                }
              >
                北海道
              </span>
            ...
            {Object.keys(legions).map((legion, index) => (
              <div key={index} className="flex gap-[22px]">
                <span
                  className="text-[#02540A] w-[48px] pb-[10px] cursor-pointer"
                  onClick={(e) => goToItemListPageWithLegion(e)}
                >
                  {legion}
                </span>
                <div
                  onClick={(e) =>
                    goToItemListPageWithProvinceAddedFilterContent(e)
                  }
                  className=" cursor-pointer"
                >
                  {legions[legion].map((province, index) => (
                    <span key={index} className="pr-[5px]">
                      {province}
                    </span>
                  ))}
                </div>
        ...
        <div
          className={`absolute top-32 right-10 text-[16px] font-medium border-b-2 border-b-emerald-600 py-1 px-3 cursor-pointer ${
            !isXL ? "hidden" : displayAgents ? "hidden" : "block"
          }`}
          onClick={() => handleDisplayAgentsToggle(true)}
        >
          <div className="pr-3 fa-solid fa-arrow-left"></div>
          エージェントを見る
        </div>
        {displayAgents && (
      ...
      <div
        className={`grid lg:hidden gap-x-4 gap-y-4 sm:gap-y-8 md:gap-y-12 mt-5 mx-auto box-border max-w-[500px] grid-cols-3`}
      >
        {Object.keys(myImage).map((key, i) => (
            <div
      ...
      <div
        ...
        onClick={() => handleDisplayAgentsToggle(true)}
      >
        エージェントを見る<div className="pl-2 fa-solid fa-arrow-down"></div>
      </div>
      {displayAgents && (
        <div className="flex flex-col items-center xl:hidden w-[350px] sm:w-[500px] md:w-[720px] relative mt-4 bg-white shadow-lg rounded-2xl">
          <div
            className="absolute top-4 right-6 text-sm font-bold cursor-pointer"
            onClick={() => handleDisplayAgentsToggle(false)}
          >
