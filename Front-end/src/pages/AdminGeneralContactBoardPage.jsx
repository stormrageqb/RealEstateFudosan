...
import AccordionContactMessage from "../components/AccordionContactMessage";
...
const AdminGeneralContactBoardPage = () => {
  ...
  const fetchContactMessages = async () => {
    ...
    try {
      ...
      const res = await axios.get(`fetchContactMessagesByAdmin?${params}`);
  ...
  const handleMessageItemClicked = (props) => {
    const index = props;
    const clientId = messages[index].clientId._id;
    console.log('clientId--------', clientId);
    const params = new URLSearchParams({ clientId: clientId }).toString();
    history.push(`contact-general?${params}`);
  };
        ...
        <div className="flex flex-col items-center w-full min-h-[500px]">
          ...
          {messages.map((msg, index) => {
            return (
              ...
              >
                <AccordionContactMessage key={index} message={msg} />
              </div>
            );
          })}
        </div>
...
export default AdminGeneralContactBoardPage;
