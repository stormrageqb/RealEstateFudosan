...
const AdminViewAgentPage = () => {
    ...
    return (
        <div className='relative flex flex-col items-center w-full pt-20 bg-[#F1F1F1] noto-regular'>
            <div className='text-center text-[32px] noto-semibold'>エージェントを見る</div>
            ...
            {
                agents.length === 0 &&
                <div className='pt-[200px] text-3xl text-center noto-medium'>ここに掲載されたエージェントはありません。</div>
            }
            <div className={`grid gap-x-16 gap-y-5 max-w-[1250px] min-h-[350px] pt-10 pb-24
            ${agents.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                {
                    agents.map((agent, index) => {
                        ...
                        return (
                            ...
                            <div key={index} className='flex flex-col items-center w-[500px] px-4 py-4 bg-white rounded-lg border-[1px] border-black/30 cursor-pointer' onClick={() => handleAgentClicked(index)}>
                                <div className=''> {agent.agentName.firstNameGanji}さん,  {agent.companyName}</div>
                                ...
                                <div className='flex justify-between gap-4 mt-4'>
                                    <button className='w-[150px] py-1 text-white bg-[#2A6484] text-sm noto-medium rounded-xl cursor-pointer ' onClick={(e) => handleContactPosterButtonClicked(e, agent)}>投稿者への連絡</button>
                                    <button className='w-[150px] py-1 text-[#2A6484] bg-white text-sm noto-medium rounded-xl cursor-pointer border-[2px] border-[#2A6484]' onClick={(e) => { handleApproveToggleBtnClicked(e, agent) }}>{approved ? '掲示板に削除する' : '掲示板に投稿する'}</button>
                                </div>
                            </div>
                        )
            ...
            {
                showApproveAgentModal &&
                <ApproveAgentModal agent={selectedAgent} handleToggleShowModal={handleToggleShowModal} />
            }
        </div>
    );
}
