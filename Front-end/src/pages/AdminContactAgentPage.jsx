...
const AdminContactAgentPage = () => {
  ...
  return (
    <div className="flex flex-col items-center w-full bg-[#F1F1F1] pt-20 font-normal">
      <div className="flex flex-col items-center w-[500px] px-4 py-3 mb-16 bg-white rounded-lg border-[1px] border-black/30 cursor-pointer">
        ...
        <div className="text-xs pt-3 max-h-[100px] overflow-y-auto">
          {agent.content}
        </div>
      </div>
      {contactMessages.length === 0 && (
        <div className="w-full min-h-[150px] mt-[120px] text-[32px] font-medium text-center">
          まだメッセージはありません
        </div>
      )}
      {contactMessages.map((message, index) => (
        ...
        <div
          key={index}
          className={`w-[900px] h-auto border-2 border-[#2A6484]/40 p-4 mb-4 ${
      ...
      <div className="flex justify-center pt-[48px] pb-[45px]">
        <textarea
          ref={textareaRef}
          className="border-[1px] border-black text-sm rounded-sm pt-1 pl-2"
          name="message"
          id=""
          cols="100"
          rows="4"
      ...
      <div className="flex justify-center mb-[100px]">
        <button
          className="py-[20px] px-[50px] rounded-2xl bg-[#2A6484] text-[20px] text-white font-medium"
