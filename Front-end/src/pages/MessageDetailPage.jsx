...
const MessageDetailPage = () => {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <p className="text-[24px] md:text-[32px] font-semibold text-center pt-20">
        サスケさんとのメッセージ
      </p>
      <div className="flex flex-col items-center w-full min-h-[300px] mt-20">
        ...
        {previousPage === "itemBoardPage" && <AgentCard agent={agent} />}
        <div className="flex flex-col w-[90%] sm:w-[600px] md:w-[750px] lg:w-[900px] pt-5 gap-4">
          {messages.map((msg, index) => {
            return <AccordionItemMessage key={index} message={msg} />;
          })}
        </div>
        {messages.length === 0 && (
      ...
      <textarea
        ref={textareaRef}
        className="w-[85%] md:w-[580px] lg:w-[750px] xl:w-[850px] border-[1px] border-black pt-1 pl-2 rounded-sm my-16"
        name="message"
        id=""
        cols=""
        rows="4"
      ...
      <div className="flex justify-center mb-[100px]">
        {myId === opponentId ? (
          <button
            className="py-[20px] px-[50px] rounded-2xl bg-gray-500 font-medium text-xl text-white"
            disabled={true}
            onClick={handleSendMessageBtnClicked}
          >
            メッセージを送信する
          </button>
        ) : (
